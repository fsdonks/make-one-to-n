...
def replace_srcs(old_df, new_df, value_list, out_path, key='SRC',):
    ...
    print("values being replaced: ", value_list)

def replace_srcs_csv(old_file, new_file, value_list, out_path, key='SRC',):
    new_df=pd.read_csv(new_file, sep='\t')
    old_df=pd.read_csv(old_file, sep='\t')
    replace_srcs(old_df, new_df, value_list, out_path, key)

def add_branch(df):
    df['branch']=df['SRC'].str[:2]
    
def replace_branches(old_file, new_file, value_list, out_path):
    new_df=pd.read_csv(new_file, sep='\t')
    new_df=add_branch(new_df)
    old_df=pd.read_csv(old_file, sep='\t')
    old_df=add_branch(old_df)
    replace_srcs(old_df, new_df, value_list, out_path, 'branch')
...
def test_1(repo_path):  
    ...
    replace_srcs_csv(three_periods, two_periods, ['01205K000'], out)
...
def test_2(repo_path):
    ...
    replace_srcs_csv(big_phases, two_periods, ['01205K000'], out)
...
def test_3(repo_path):
    ...
    replace_srcs_csv(big_phases, no_truncation, ['01205K000', "01225K000"], out)
    ...
    replace_srcs_csv(big_phases, four_srcs, False, out_2)
